...
const options : { connection: string, out: string, tables: string, bookshelf: string} = commandLineArgs(optionSpec)

Promise.all(options.tables.split(',').map(table => fetchTable(options.connection, table.trim() )))
  .then(tables =>
    Promise.all(tables.map(t =>
      new Promise((ok, fail) => {
        const model = createModel(options.bookshelf, t, tables)
